...
namespace 任务管理器x频谱分析器 {
	unsafe public static class FFTTools {
		...
		private static double Max(ReadOnlySpan<double> data, double index1, double index2) {
			...
			//if (ix2 < ix1) {
			//	return (Interpolation(data, index1) + Interpolation(data, index2)) / 2 * (index2 - index1);
			//}

			//double sum = 0;
			//fixed (double* p = data) {
			//	sum += (Interpolation(data, index1) + p[ix1]) / 2 * (ix1 - index1);
			//	for (int i = ix1; i < ix2; i++) {
			//		sum += (p[i] + p[i + 1]) / 2;
			//	}
			//	sum += (p[ix2] + Interpolation(data, index2)) / 2 * (index2 - ix2);
			//}
			//return sum;
